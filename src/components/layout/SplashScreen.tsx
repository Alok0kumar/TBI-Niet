...
export default function SplashScreen({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Smoke animations take about 1.8s to fully complete and fade to opacity 0
  ...
  return (
      ...
      <AnimatePresence>
        {(phase !== "reveal" && phase !== "done") && (
          <motion.div
            ...
            style={{ background: "rgba(0, 0, 0, 1)" }} // Solid black so website is hidden
            exit={{ opacity: 0 }}
    ...
    height={144}
            ...
            {/* Massive Billowing Smoke Clouds (Fixed to bottom, exploding upwards/outwards) */}
            <div className="absolute bottom-0 left-0 right-0 h-10 flex justify-center items-end pointer-events-none z-[-1]">
              ...
              {/* Center Main Cloud Expansion (Covers the whole screen) */}
              <motion.div
                className="absolute rounded-full"
                style={{
                ...
                animate={
                  phase === "rumble"
                    ...
                    ? {
                        opacity: [0.9, 1, 1, 0],
                        scale: [2, 15, 30], // Massive scale to cover the whole screen
                        y: [0, -500, -1500], // Expands massively upward
                        transition: { duration: 1.5, ease: "easeOut" },
                      }
                    : { opacity: 0, scale: 0 }
                }
